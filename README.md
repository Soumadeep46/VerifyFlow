...
## Compliance-Preserving Merchant Verification and Revenue Unlock Agent

VerifyFlow is an AI-assisted merchant verification and revenue-recovery prototype built for the Razorpay Buildathon.
...
> **Important:** This project uses entirely synthetic data. It is not connected to Razorpay production systems and does not perform live PAN, GSTIN, bank, sanctions, or government verification.

---

## Problem

Merchant onboarding can be delayed by issues such as:
...
Not every exception should be handled in the same way. Some issues can be safely corrected by requesting better evidence, while others require human review or a hard compliance block.

VerifyFlow separates these outcomes instead of treating every exception as an automatic approval or rejection.

---

## Solution

VerifyFlow uses a layered verification workflow:
...
Each case is classified into one of three categories:

### 1. SAFE_TO_REMEDIATE

Issues that can potentially be resolved through bounded evidence correction.
...
The `REVENUE_UNLOCKED` state represents an estimated business-impact outcome in the prototype. It should be interpreted as the merchant becoming eligible to accept payments after successful verification, not as a claim of actual recovered Razorpay revenue.
...
- Deterministic verification rules
- Document-quality and simulated OCR checks
...
- Safe fallback when no AI API key is configured
...
> The large dataset generator and large generated datasets are intentionally excluded from the public repository where applicable. The repository contains the application code and the files required for a lightweight demonstration.
...
VerifyFlow estimates potential revenue impact using a synthetic formula:
...
The dashboard compares VerifyFlow with a synthetic FIFO baseline.
...
VerifyFlow is designed with the following safeguards:
...
VerifyFlow is an independent buildathon prototype.
