...
st.set_page_config(page_title="VerifyFlow", layout="wide")
...
with tab_detail:
    ...
    if case.exception_type == ExceptionType.SAFE_TO_REMEDIATE and case.document_issues:
        ...
        if st.button("Re-run Verification", key=f"rerun_{case.case_id}"):
            result = rerun_verification(case, passed=True)
...
with tab_benchmark:
    ...
    col2.metric("VerifyFlow Avg (hrs)", f"{summary['verifyflow_avg_hours']:.2f}")
    ...
    st.caption("FIFO baseline and VerifyFlow figures reflect the synthetic dataset in this demo, not production Razorpay metrics.")
